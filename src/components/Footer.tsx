...
function SocialIcon({ type, className }: { type: string; className?: string }) {
  const icons: Record<string, JSX.Element> = {
    ...
    twitter: <svg viewBox="0 0 24 24" fill="currentColor" className={className}><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>,
    youtube: <svg viewBox="0 0 24 24" fill="currentColor" className={className}><path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>,
    ...
    <footer className="border-t border-border bg-foreground text-primary-foreground">
      <div className="container py-12 md:py-16">
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <img src={temboLogo} alt="African Tembo Safari" className="h-10 w-10 rounded-full object-cover" />
              <span className="font-display text-lg font-bold">African Tembo Safari</span>
            </div>
            <p className="text-sm text-primary-foreground/70">
              Born from the red earth of the savannah. We curate authentic, expert-led journeys through the heart of Tanzania — honoring the Tembo with every step.
            </p>
              ...
              <div className="flex items-center gap-3 pt-1">
                {activeSocials.map(([type, url]) => (
                  <a
                    ...
                    aria-label={type}
                  >
                    <SocialIcon type={type} className="h-5 w-5" />
                  </a>
                ))}
              </div>
            )}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2 text-sm text-primary-foreground/70">
              <div className="flex items-start gap-2"><MapPin className="h-4 w-4 shrink-0 mt-0.5" />{address}</div>
              <a href={`tel:${phone}`} className="flex items-center gap-2 hover:text-primary-foreground transition-colors">
        ...
        <div className="mt-10 border-t border-primary-foreground/10 pt-6 text-center text-xs text-primary-foreground/50">
          © {new Date().getFullYear()} African Tembo Safari. All rights reserved. The wild is waiting for you.
        </div>
      </div>
    </footer>
  );

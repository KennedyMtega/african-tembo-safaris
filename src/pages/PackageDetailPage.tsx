...
export default function PackageDetailPage() {
  ...
  const related = mockPackages.filter((p) => p.id !== pkg.id && p.status === "published").slice(0, 3);
  const [mainImage, ...thumbs] = pkg.images;
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
                ...
                <span className="flex items-center gap-1"><Star className="h-4 w-4 text-safari-gold" />{pkg.rating} ({pkg.reviewCount} reviews)</span>
              </div>
            </div>
            ...
            <Card className="sticky top-24 border-border/50">
              <CardContent className="p-6 space-y-4">
                <div>
                  ...
                  <p className="font-display text-3xl font-bold text-primary">${pkg.price.toLocaleString()}</p>
                  <p className="text-sm text-muted-foreground">per person</p>
                </div>
                {pkg.groupPrice && (
                  <div className="rounded-md bg-secondary/50 p-3">
                    <p className="text-sm font-semibold text-foreground">Group Discount</p>
                    <p className="text-sm text-muted-foreground">${pkg.groupPrice.toLocaleString()} per person for 4+ travelers</p>
                  </div>
                )}
